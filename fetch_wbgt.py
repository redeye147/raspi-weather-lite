...
更新頻度: 環境省は 1 時間ごと（本モジュールは呼び出し側でキャッシュすること）
"""
...
def _fetch_alert(pref_code: str) -> bool:
    ...
    # 環境省アラート一覧ページから発令中の都道府県を確認
    url = "https://www.wbgt.env.go.jp/alert.php"
    try:
        r = requests.get(url, headers=_HEADERS, timeout=10)
        r.raise_for_status()
        # 都道府県コードが含まれているかで判定（簡易スクレイピング）
        # 例: pref_code=23 (愛知) → "23" が alert area として出現
        text = r.text
